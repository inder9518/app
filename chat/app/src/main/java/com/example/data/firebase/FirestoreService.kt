...
object FirestoreService {
                        ...
                        if ((sender == userA && receiver == userB) || (sender == userB && receiver == userA)) {
                            list.add(
                                ChatMessage(
                                    ...
                                    isSeen = doc.getBoolean("isSeen") ?: false
                                )
                            )
                        }
                    }
    ...
    // Mark messages as seen/read in a single transaction/batch
    fun markMessagesAsSeen(receiverId: String, senderId: String) {
        try {
            val db = getDb()
                    ...
                    if (snapshot != null && !snapshot.isEmpty) {
                        ...
                        batch.commit()
                            .addOnFailureListener { e ->
                                Log.e(TAG, "Failed to commit seen batch: ${e.message}")
                            }
                    }
                }
                .addOnFailureListener { e ->
                    Log.e(TAG, "Failed to search messages to mark seen: ${e.message}")
                }
        } catch (e: Exception) {
